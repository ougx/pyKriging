...
!
! 2026/05/03 mou@sspa.com: Added kdtree2_n_nearest_maxidx and kdtree2_r_nearest_maxidx.
!   These are variants of kdtree2_n_nearest and kdtree2_r_nearest that accept
!   an integer max_idx argument and only return points with index <= max_idx.
!   Intended for sequential Gaussian simulation (SGS) where observations occupy
!   indices 1..n_obs and grid nodes n_obs+1..n_total, visited in index order.
!   Original routines and tree_search_record are unchanged.
...
module kdtree2_module
  ...
  !
  public :: kdtree2_n_nearest_maxidx
  ! Like kdtree2_n_nearest but only considers points with index <= max_idx.
  !
  public :: kdtree2_r_nearest, kdtree2_r_nearest_around_point
  ! Return points within a fixed ball of arb vector/extant point
  !
  public :: kdtree2_r_nearest_maxidx
  ! Like kdtree2_r_nearest but only considers points with index <= max_idx.
  !
  ...
  type :: tree_search_record
      ...
      integer           :: max_idx = huge(0)
      ! only include points with original index <= max_idx (used by _maxidx routines)
...
!          dis=extra**2
  ...
  ! -----------------------------------------------------------------------
  ! max_idx variants: only search points with original index <= max_idx.
  !
  ! Intended for sequential Gaussian simulation where data is laid out as:
  !   indices 1..n_obs          : observations (always included)
  !   indices n_obs+1..n_total  : grid nodes (included once estimated)
  !
  ! Usage:
  !   call kdtree2_n_nearest_maxidx(tp, qv, nn, results, max_idx=n_obs+k)
  !   call kdtree2_r_nearest_maxidx(tp, qv, r2, nfound, nalloc, results, max_idx=n_obs+k)
  ! -----------------------------------------------------------------------

  subroutine kdtree2_n_nearest_maxidx(tp, qv, nn, results, max_idx)
    ! Find the 'nn' nearest neighbors to 'qv', considering only points
    ! with original index <= max_idx.
    type(kdtree2), pointer               :: tp
    real(kdkind), target, intent(in)     :: qv(:)
    integer, intent(in)                  :: nn
    type(kdtree2_result), target         :: results(:)
    integer, intent(in)                  :: max_idx
    ...
    call search_maxidx(tp%root)

    if (tp%sort) call kdtree2_sort_results(nn, results)
  end subroutine kdtree2_n_nearest_maxidx


  subroutine kdtree2_r_nearest_maxidx(tp, qv, r2, nfound, nalloc, results, max_idx)
    ! Find all neighbors of 'qv' within squared distance r2, considering
    ! only points with original index <= max_idx.
    type(kdtree2), pointer               :: tp
    real(kdkind), target, intent(in)     :: qv(:)
    real(kdkind), intent(in)             :: r2
    integer, intent(out)                 :: nfound
    ...
    sr%qv        => qv
    sr%ballsize  = r2
    sr%nn        = 0     ! flag: fixed-ball search
    sr%nfound    = 0
    sr%centeridx = -1
    sr%correltime = 0
    sr%max_idx   = max_idx
    ...
    if (sr%overflow) then
       write(*,*) 'KD_TREE_TRANS: warning! kdtree2_r_nearest_maxidx found more neighbors'
       write(*,*) 'KD_TREE_TRANS: than storage provided. Answer is NOT the smallest ball.'
    endif
  end subroutine kdtree2_r_nearest_maxidx


  recursive subroutine search_maxidx(node)
    ! Identical tree traversal to search(), but dispatches to
    ! process_terminal_node_maxidx / process_terminal_node_fixedball_maxidx.
    type(tree_node), pointer   :: node
    type(tree_node), pointer   :: ncloser, nfarther
    integer                    :: cut_dim, i
    real(kdkind)               :: qval, dis, ballsize
    real(kdkind), pointer      :: qv(:)
    type(interval), pointer    :: box(:)

    if ((associated(node%left) .and. associated(node%right)) .eqv. .false.) then
       if (sr%nn .eq. 0) then
          call process_terminal_node_fixedball_maxidx(node)
       else
          call process_terminal_node_maxidx(node)
       endif
    else
       qv => sr%qv(1:)
       cut_dim = node%cut_dim
       ...
       if (associated(ncloser)) call search_maxidx(ncloser)
       ...
       if (associated(nfarther)) then
          ...
          if (dis <= ballsize) then
             ...
             call search_maxidx(nfarther)
          endif
       endif
    end if
  end subroutine search_maxidx


  subroutine process_terminal_node_maxidx(node)
    ! Like process_terminal_node but skips points with index > sr%max_idx.
    type(tree_node), pointer    :: node
    real(kdkind), pointer       :: qv(:)
    integer, pointer            :: ind(:)
    real(kdkind), pointer       :: data(:,:)
    ...
    mainloop: do i = node%l, node%u
       indexofi = ind(i)
       if (indexofi > sr%max_idx) cycle mainloop   ! index filter
       if (rearrange) then
          sd = 0.0
          do k = 1, dimen
             sd = sd + (data(k,i) - qv(k))**2
    ...
    sr%ballsize = ballsize
  end subroutine process_terminal_node_maxidx


  subroutine process_terminal_node_fixedball_maxidx(node)
    ! Like process_terminal_node_fixedball but skips points with index > sr%max_idx.
    type(tree_node), pointer    :: node
    real(kdkind), pointer       :: qv(:)
    integer, pointer            :: ind(:)
    real(kdkind), pointer       :: data(:,:)
    integer                     :: nfound, dimen, i, indexofi, k, nn
    real(kdkind)                :: ballsize, sd
    logical                     :: rearrange
